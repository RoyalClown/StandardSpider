"""
    @description:   来源:Rohm
                    ...
                    目标类目:多输出低饱和稳压器(LDO)
                    商城类目:低压差线性稳压器
                    来源网址:http://www.rohm.com.cn/web/china/search/parametric/-/search/Multi-Output%20LDO%20Regulators
    @author:        RoyalClown
    @date:          2016/11/21
"""
import re
...
class ProductList:
    ...
    def get_urls_pdfs(self):
        with open("I:\PythonPrj\StandardSpider\Spider\Rohm\VoltageRegulator\Single_Output_LDO\htmlcode.html", "r",
                  encoding="utf-8") as f:
            content = f.read()
...
class Detail:
    ...
    def get_component(self):
        url = self.url
        code = self.code
        kiname = "单输出低饱和稳压器(LDO)"
        attach = self.pdf
        rough_img = re.search(r'productThumnailImageLogo="(http://ro hmfs\.rohm\.com.*?\.jpg)"', self.content)
        try:
...
if __name__ == "__main__":
    ...
    pdfs_urls = productlist.get_urls_pdfs()
