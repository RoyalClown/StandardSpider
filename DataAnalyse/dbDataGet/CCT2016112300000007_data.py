            ...
            for base_property in base_properties:
                ...
                for crawl_property in crawl_base_properties:
                    ...
                    if crawl_property_name.lower().replace(" ", "") in aim_property_name.lower().replace(" ", ""):
                        ...
                        # 针对参数选择
                        # 获取最小值
                        if crawl_property_name == "Output Current":
                            tmp_pv_min = crawl_property_value
                            if crawl_property_value:
                                numberic = tmp_pv_min
                                save_value = numberic + base_property_unit_list[1]
                                ...
                                property_json = spcap_data.get_property_json(base_property_detno, pv_id,
                                                                             ...
                                                                             numberic=numberic,
                                                                             unit=base_property_unit_list[1])
                                break
                            elif tmp_pv_max:
                                numberic = tmp_pv_max
                                save_value = numberic + base_property_unit_list[0]
                                pv_id = spcap_data.save_to_property(base_property_id, component_id,
                                                                    base_property_detno,
                                                                    "'" + save_value + "'",
                                                                    pv_numberic=numberic,
                                ...
                                property_json = spcap_data.get_property_json(base_property_detno, pv_id,
                                                                             ...
                                                                             base_property_name, save_value,
                                                                             numberic=numberic,
                                                                             unit=base_property_unit_list[0])
                                break
                            else:
                                tmp_pv_min = 'null'
                                continue
                        # 获取最大值
                        elif crawl_property_name == "OutputCurrent(mA)":
