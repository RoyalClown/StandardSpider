"""
    ...
    @date:          2016/11/17
"""
from DataAnalyse.dbDataGet.Util_data import UtilDataAnalyse
    ...
    def go(self, task_id):
        ...
        for crawl_component in crawl_components:
            ...
            component_id = spcap_data.save_to_component(crawl_component_code, b2c_kiid, b2c_brid, uuid,
                                                        ...
                                                        version=cmp_version)
            # 更新爬虫表返回uuid
            spcap_data.update_crawl_uuid(uuid, task_id, crawl_component_code, cc_flag)
            #
            ...
            for base_property in base_properties:
                ...
                for crawl_property in crawl_base_properties:
                    ...
                    if crawl_property_name.lower().replace(" ", "") in aim_property_name.lower().replace(" ", ""):
                        ...
                        """针对参数合并为范围值"""
                        # 获取最大值
                        if crawl_property_name == "OperatingTemperatureMin_num":
                            temperature_min = crawl_property_value
                            continue
                        # 获取最小值并储存为范围值
                        elif crawl_property_name == "OperatingTemperatureMax_num":
                        ...
                        """针对参数合并为单个值"""
                        if crawl_property_name == "PS_PackagePins":
                            tmp_pv_min = crawl_property_value.replace("-pin", "")
                            continue
                        elif crawl_property_name == "PS_PackageShortCode":
                            tmp_pv_max = crawl_property_value
                            save_value = tmp_pv_max + tmp_pv_min
                        ...
                        """针对参数值处理"""
                        if aim_property_name == "PS_CurrentConsumption":
                            pv_numberic = crawl_property_value.split("-")[0]
                            save_value = pv_numberic + base_property_unit
                            pv_id = spcap_data.save_to_property(base_property_id, component_id,
                                                                base_property_detno,
                                                                "'" + save_value + "'",
                                                                pv_numberic=pv_numberic,
                                                                pv_unit="'" + base_property_unit + "'")
                            property_json = spcap_data.get_property_json(base_property_detno, pv_id,
                                                                         base_property_id,
                                                                         base_property_name, save_value,
                        ...
                        if aim_property_name == "PS_Sensitivity":
                            if base_property_unit_list[0] in crawl_property_value:
                                pv_unit = base_property_unit_list[0]
                                ...
                                pv_unit = base_property_unit_list[1]
                                save_value = crawl_property_value
                        ...
                        """针对参数分割"""
                        if crawl_property_name == "PS_SupplyVoltage":
                            voltage_min_max = crawl_property_value.split("-")
                            # 只有一个数值
                            if len(voltage_min_max) == 1:
                                voltage_min = crawl_property_value
                                voltage_max = crawl_property_value
                            else:
                                voltage_min = voltage_min_max[0]
                                voltage_max = voltage_min_max[1]
                            if base_property_name == "电源电压-最大":
                                pv_numberic = voltage_max
                            else:
                                pv_numberic = voltage_min
                            save_value = pv_numberic + base_property_unit
                            pv_id = spcap_data.save_to_property(base_property_id, component_id, base_property_detno,
                                                                "'" + save_value + "'",
                                                                pv_numberic=pv_numberic,
