...
class UtilDataAnalyse(OracleConnection):
    ...
    def save_to_component(self, code, kiid, brid, cmp_uuid, attach, img, version=1):
        ...
        cursor.execute(
            "insert into product$component(cmp_id, cmp_code,cmp_unit,cmp_version,cmp_kiid,cmp_brid,cmp_uuid,cmp_attach,cmp_img) values({},'{}','PCS',{},{},{},'{}','{}','{}')".format(
                component_id, code, version, kiid, brid, cmp_uuid, attach, img))
        cursor.close()
        return component_id
    ...
    def update_crawl_uuid(self, uuid, taskid, code, cc_modify, cc_flag):
        cursor = self.conn.cursor()
        cursor.execute(
            "update product$component_crawl set cc_uuid='{}', cc_flag={}, cc_modify={} where cc_task={} and cc_code='{}' and cc_flag is not null".format(
