                ...
                for crawl_property in crawl_base_properties:
                    ...
                    if crawl_property_name.lower().replace(" ", "") in aim_property_name.lower().replace(" ", "") or (aim_property_name == "电阻值容差" and crawl_property_name == "电阻值容差 (%)" )\
                        ...
                        if crawl_property_name == "直流或交流":
                            tmp_voltage_AC_DC = crawl_property_value
                            continue
                        # 获取额定电压 (V)
                        elif aim_property_name == "电压额定值 AC":
                            if tmp_voltage_AC_DC == 'AC':
                                ...
                                continue
                        elif aim_property_name == "电压额定值 DC":
                            if tmp_voltage_AC_DC == 'DC':
                                numberic = crawl_property_value
                                save_value = numberic + base_property_unit
                                pv_id = spcap_data.save_to_property(base_property_id, component_id,
                                                                    base_property_detno,
                        ...
                        # 储存为范围值
                        # 获取最小值
                        if crawl_property_name == "长度尺寸 (mm)":
                            tmp_three_first = crawl_property_value
                            continue
                        # 获取典型值
                        elif crawl_property_name == "厚度尺寸 (mm)":
                            tmp_three_second = crawl_property_value
                            continue
                        # 获取最大值
                        elif crawl_property_name == "高度尺寸 (mm)":
                            tmp_three_third = crawl_property_value
                            if tmp_three_first != '':
                                pv_min = tmp_three_first
                                pv_max = tmp_three_second
                                save_value = pv_min + 'L*' + pv_max + 'T*' + tmp_three_third + 'H'
                                pv_id = spcap_data.save_to_property(base_property_id, component_id,
                                                                    base_property_detno,
